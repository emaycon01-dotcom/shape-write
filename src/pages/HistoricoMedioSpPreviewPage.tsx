...
const FORM_ROUTE = "/dashboard/documents/historico-medio-sp";

type Payload = { pdfBase64: string; formData: Record<string, unknown> };

export default function HistoricoMedioSpPreviewPage() {
  ...
  useEffect(() => {
    ...
    const id = window.setTimeout(() => {
      prefetchGeneratePdf("generate-historico-medio-sp-pdf", { ...formData, preview: false });
    }, 1200);
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const deduction = await deductCredit(1, "geracao-historico-medio-sp", creditRef("geracao-historico-medio-sp", formData));
      if (!deduction.ok) {
      ...
      await addDocument({
        name: txt("nome_aluno"),
        identification: txt("periodo_conclusao"),
        date: txt("data_nascimento"),
        description: `HISTÓRICO ESCOLAR (ENSINO MÉDIO) - ${txt("escola")}`,
  ...
  const handleDownload = () => {
    try {
      ...
      link.href = url;
      link.download = "historico-escolar-ensino-medio.pdf";
      link.click();
  ...
  const handleShare = async () => {
    try {
      const blob = pdfDataUrlToBlob(pdfBase64);
      if (!blob) throw new Error("Failed to create PDF blob");
      const file = new File([blob], "historico-escolar-ensino-medio.pdf", { type: "application/pdf" });
      if (navigator.share && navigator.canShare({ files: [file] })) {
        await navigator.share({ files: [file], title: "HISTÓRICO ESCOLAR — ENSINO MÉDIO" });
      } else {
        handleDownload();
      }
    } catch {
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      <div className="glass relative mb-6 overflow-hidden rounded-xl" style={{ height: "70vh" }}>
        <PdfCanvasPreview pdfDataUrl={pdfBase64} title="Preview do HISTÓRICO ESCOLAR" />
