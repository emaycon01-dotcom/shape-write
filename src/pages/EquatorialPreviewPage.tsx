...
export default function EquatorialPreviewPage() {
  ...
  useEffect(() => {
    ...
    const id = window.setTimeout(() => {
      prefetchGeneratePdf("generate-equatorial-pdf", { ...formData, preview: false });
    }, 1200);
  ...
  if (!pdfBase64 || !formData) {
    return (
      <div className="flex flex-col items-center justify-center gap-4 py-20">
        ...
        <Button variant="outline" onClick={() => navigate("/dashboard/documents/comprovante-equatorial")}>
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      await addDocument({
        ...
        description: `Comprovante de Residência - Equatorial Goiás ${formData.municipio || ""}`,
  ...
  const handleDownload = () => {
    try {
      ...
      link.href = url;
      link.download = "comprovante-residencia-equatorial.pdf";
      link.click();
  ...
  const handleShare = async () => {
    try {
      const blob = pdfDataUrlToBlob(pdfBase64);
      if (!blob) throw new Error("Failed to create PDF blob");
      const file = new File([blob], "comprovante-residencia-equatorial.pdf", { type: "application/pdf" });
  ...
  return (
    <div className="mx-auto max-w-2xl">
      <button
        onClick={() => navigate("/dashboard/documents/comprovante-equatorial")}
      ...
      <h1 className="font-display mb-1 text-2xl font-bold text-foreground">
        {paid ? "Documento Gerado" : "Preview do Comprovante — Equatorial Goiás"}
      </h1>
      ...
      <div className="glass relative mb-6 overflow-hidden rounded-xl" style={{ height: "70vh" }}>
        <PdfCanvasPreview pdfDataUrl={pdfBase64} title="Preview do Comprovante de Residência Equatorial Goiás" />
