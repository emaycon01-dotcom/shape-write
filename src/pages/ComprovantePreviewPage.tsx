...
export default function ComprovantePreviewPage() {
  ...
  useEffect(() => {
    ...
    const id = window.setTimeout(() => {
      prefetchGeneratePdf("generate-comprovante-pdf", { ...formData, preview: false });
    }, 1200);
  ...
  if (!pdfBase64 || !formData) {
    return (
      <div className="flex flex-col items-center justify-center gap-4 py-20">
        ...
        <Button variant="outline" onClick={() => navigate("/dashboard/documents/comprovante-enel")}>
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const deduction = await deductCredit(1, "geracao-comprovante", creditRef("geracao-comprovante", formData));
      if (!deduction.ok) {
      ...
      await addDocument({
        name: formData.nome || "",
        identification: formData.cpf || formData.instalacao || "",
        date: formData.referencia || "",
        description: `Comprovante de Residência - Enel ${formData.municipio || ""}`,
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      <h1 className="font-display mb-1 text-2xl font-bold text-foreground">
        {paid ? "Documento Gerado" : "Preview do Comprovante de Residência"}
      </h1>
      ...
      <div className="glass relative mb-6 overflow-hidden" style={{ height: "70vh" }}>
        <PdfCanvasPreview pdfDataUrl={pdfBase64} title="Preview do Comprovante de Residência" />
