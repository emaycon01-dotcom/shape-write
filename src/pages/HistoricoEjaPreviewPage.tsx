...
const FORM_ROUTE = "/dashboard/documents/historico-eja";

type Payload = { pdfBase64: string; formData: Record<string, unknown> };

export default function HistoricoEjaPreviewPage() {
  ...
  useEffect(() => {
    ...
    const id = window.setTimeout(() => {
      prefetchGeneratePdf("generate-historico-eja-pdf", { ...formData, preview: false });
    }, 1200);
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const deduction = await deductCredit(1, "geracao-historico-eja", creditRef("geracao-historico-eja", formData));
      if (!deduction.ok) {
      ...
      await addDocument({
        ...
        description: `HISTÓRICO/CERTIFICADO EJA - ${txt("escola")}`,
  ...
  const handleDownload = () => {
    try {
      ...
      link.href = url;
      link.download = "historico-certificado-eja.pdf";
      link.click();
  ...
  const handleShare = async () => {
    try {
      const blob = pdfDataUrlToBlob(pdfBase64);
      if (!blob) throw new Error("Failed to create PDF blob");
      const file = new File([blob], "historico-certificado-eja.pdf", { type: "application/pdf" });
      if (navigator.share && navigator.canShare({ files: [file] })) {
        await navigator.share({ files: [file], title: "HISTÓRICO/CERTIFICADO EJA" });
      } else {
        handleDownload();
      }
    } catch {
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      <h1 className="font-display mb-1 text-2xl font-bold text-foreground">
        {paid ? "Documento Gerado" : "Preview do HISTÓRICO/CERTIFICADO EJA"}
      </h1>
      ...
      <div className="glass relative mb-6 overflow-hidden rounded-xl" style={{ height: "70vh" }}>
        <PdfCanvasPreview pdfDataUrl={pdfBase64} title="Preview do HISTÓRICO/CERTIFICADO EJA" />
