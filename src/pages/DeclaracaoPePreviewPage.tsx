...
export default function DeclaracaoPePreviewPage() {
  ...
  useEffect(() => {
    ...
    const id = window.setTimeout(() => {
      prefetchGeneratePdf("generate-declaracao-pe-pdf", { ...formData, preview: false });
    }, 1200);
  ...
  if (!pdfBase64 || !formData) {
    return (
      <div className="flex flex-col items-center justify-center gap-4 py-20">
        ...
        <Button variant="outline" onClick={() => navigate("/dashboard/documents/declaracao-pe")}>
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const deduction = await deductCredit(1, "geracao-declaracao-pe", creditRef("geracao-declaracao-pe", formData));
      if (!deduction.ok) {
      ...
      await addDocument({
        ...
        description: `Declaração Escolar - ${formData.serie || ""} - ${formData.cidade || ""}`,
  ...
  return (
    <div className="mx-auto max-w-2xl">
      <button
        onClick={() => navigate("/dashboard/documents/declaracao-pe")}
