...
export default function DeclaracaoEtePreviewPage() {
  ...
  useEffect(() => {
    ...
    const id = window.setTimeout(() => {
      prefetchGeneratePdf("generate-declaracao-ete-pdf", { ...formData, preview: false });
    }, 1200);
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const deduction = await deductCredit(1, "geracao-declaracao-ete", creditRef("geracao-declaracao-ete", formData));
      if (!deduction.ok) {
