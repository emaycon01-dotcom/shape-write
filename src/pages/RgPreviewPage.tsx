...
export default function RgPreviewPage() {
  ...
  if (!pdfBase64 || !formData) {
    return (
      <div className="flex flex-col items-center justify-center gap-4 py-20">
        ...
        <Button variant="outline" onClick={() => navigate("/dashboard/documents/rg")}>
  ...
  const handleGenerate = async () => {
    ...
    setLoading(true);
    try {
      const deduction = await deductCredit(1, "geracao-rg");
      if (!deduction.ok) {
      ...
      await addDocument({
        ...
        description: `RG Digital - ${formData.orgao_expedidor || ""}`,
  ...
  const handleDownload = () => {
    try {
      ...
      link.href = url;
      link.download = "documento-rg.pdf";
      link.click();
  ...
  const handleShare = async () => {
    try {
      const blob = base64ToBlob(pdfBase64);
      if (!blob) throw new Error("Failed to create PDF blob");
      const file = new File([blob], "documento-rg.pdf", { type: "application/pdf" });
      if (navigator.share && navigator.canShare({ files: [file] })) {
        await navigator.share({ files: [file], title: "RG Digital" });
      } else {
        handleDownload();
      }
    } catch {
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      <h1 className="font-display mb-1 text-2xl font-bold text-foreground">
        {paid ? "Documento Gerado" : "Preview do RG Digital"}
      </h1>
      ...
      <div className="glass relative mb-6 overflow-hidden rounded-xl" style={{ height: "70vh" }}>
        {pdfError ? (
          <div className="flex h-full flex-col items-center justify-center gap-4 p-8 text-center">
            ...
            <Button variant="outline" onClick={() => navigate("/dashboard/documents/rg")} className="gap-1.5">
