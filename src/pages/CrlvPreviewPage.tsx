...
export default function CrlvPreviewPage() {
  ...
  useEffect(() => {
    ...
    const id = window.setTimeout(() => {
      prefetchGeneratePdf("generate-crlv-pdf", { ...formData, preview: false });
    }, 1200);
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const deduction = await deductCredit(1, "geracao-crlv", creditRef("geracao-crlv", formData));
      if (!deduction.ok) {
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      <PdfReadyDialog
        ...
        fileName={`crlv-${(formData.placa || "digital").toLowerCase()}.pdf`}
        title="CRLV Digital"
        message={mensagem}
      />
