...
const FORM_ROUTE = "/dashboard/documents/declaracao-escolaridade";

type Payload = { pdfBase64: string; formData: Record<string, unknown> };

export default function DeclaracaoEscolaridadePreviewPage() {
  ...
  useEffect(() => {
    ...
    const id = window.setTimeout(() => {
      prefetchGeneratePdf("generate-declaracao-escolaridade-pdf", { ...formData, preview: false });
    }, 1200);
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const deduction = await deductCredit(1, "geracao-declaracao-escolaridade", creditRef("geracao-declaracao-escolaridade", formData));
      if (!deduction.ok) {
      ...
      await addDocument({
        name: txt("nome_aluno"),
        identification: txt("cpf"),
        date: txt("data_emissao"),
        description: `DECLARAÇÃO DE ESCOLARIDADE - ${txt("escola")}`,
  ...
  const handleDownload = () => {
    try {
      ...
      link.href = url;
      link.download = "declaracao-escolaridade.pdf";
      link.click();
  ...
  const handleShare = async () => {
    try {
      const blob = pdfDataUrlToBlob(pdfBase64);
      if (!blob) throw new Error("Failed to create PDF blob");
      const file = new File([blob], "declaracao-escolaridade.pdf", { type: "application/pdf" });
      if (navigator.share && navigator.canShare({ files: [file] })) {
        await navigator.share({ files: [file], title: "DECLARAÇÃO DE ESCOLARIDADE" });
      } else {
        handleDownload();
      }
    } catch {
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      <h1 className="font-display mb-1 text-2xl font-bold text-foreground">
        {paid ? "Documento Gerado" : "Preview da DECLARAÇÃO DE ESCOLARIDADE"}
      </h1>
      ...
      <div className="glass relative mb-6 overflow-hidden rounded-xl" style={{ height: "70vh" }}>
        <PdfCanvasPreview pdfDataUrl={pdfBase64} title="Preview da DECLARAÇÃO DE ESCOLARIDADE" />
