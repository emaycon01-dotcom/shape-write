...
export default function DiplomaPreviewPage() {
  ...
  if (!pdfBase64 || !formData) {
    return (
      <div className="flex flex-col items-center justify-center gap-4 py-20">
        ...
        <Button variant="outline" onClick={() => navigate("/dashboard/documents/diploma")}>
  ...
  const handleGenerate = async () => {
    ...
    setLoading(true);
    try {
      const deduction = await deductCredit(1, "geracao-diploma", creditRef("geracao-diploma", formData));
      if (!deduction.ok) {
      ...
      await addDocument({
        ...
        date: formData.data_colacao || "",
        description: `Diploma - ${formData.curso || ""}`,
  ...
  const handleShare = async () => {
    try {
      ...
      if (navigator.share && navigator.canShare({ files: [file] })) {
        await navigator.share({ files: [file], title: "Diploma" });
      } else {
        handleDownload();
      }
    } catch {
  ...
  return (
    <div className="mx-auto max-w-3xl">
      <button
        onClick={() => navigate("/dashboard/documents/diploma")}
      ...
      <h1 className="font-display mb-1 text-2xl font-bold text-foreground">
        {paid ? "Documento Gerado" : "Preview do Diploma"}
      </h1>
      ...
      <div className="glass relative mb-6 overflow-hidden rounded-xl" style={{ height: "70vh" }}>
        <PdfCanvasPreview pdfDataUrl={pdfBase64} title="Preview do Diploma" />
