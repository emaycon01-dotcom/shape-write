...
interface FormData {
  ...
  dataExpedicao: Date | undefined;
  dataNascimento: Date | undefined;
  terminoCurso: Date | undefined;
  rg: string;
}
...
export default function SegurancaEscolarFormPage() {
  ...
  const fillTest = async () => {
    setForm({
      ...
      numeroRegistro: `${generateRandom(4)}/${generateRandom(2)}`,
      dataExpedicao: randomPastDate(),
      dataNascimento: randomPastDate(),
      terminoCurso: randomPastDate(),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const templateRes = await fetch("/assets/template-carteira-seguranca-escolar.pdf");
      ...
      const frenteStorageKey = "carteirinha-seguranca-escolar-frente-field-positions";
      const versoStorageKey = "carteirinha-seguranca-escolar-verso-field-positions";
      ...
      const bodyData = {
        tipo: "seguranca-escolar",
        nome_completo: form.nomeCompleto,
        cpf: form.cpf,
        numero_registro: form.numeroRegistro,
        data_expedicao_1: formatDateBR(form.dataExpedicao),
        data_nascimento: formatDateBR(form.dataNascimento),
        termino_curso: formatDateBR(form.terminoCurso),
        rg: form.rg,
      ...
      navigate("/dashboard/documentos-fisicos/carteirinhas/seguranca-escolar/preview", {
        state: {
          pdfBase64: pdfResult,
          formData: {
            tipo: "seguranca-escolar",
            tipoLabel: "Carteira de Segurança Escolar",
            nomeCompleto: form.nomeCompleto,
            cpf: form.cpf,
            numeroRegistro: form.numeroRegistro,
            dataExpedicao: formatDateBR(form.dataExpedicao),
            dataNascimento: formatDateBR(form.dataNascimento),
            terminoCurso: formatDateBR(form.terminoCurso),
            rg: form.rg,
            foto_base64: fotoPreview || "",
          },
        },
  ...
  return (
    <div className="max-w-2xl">
      ...
      <h1 className="font-display text-2xl font-bold text-foreground mb-1">Carteira de Segurança Escolar</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="glass rounded-xl p-6 space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <Input value={form.numeroRegistro} onChange={set("numeroRegistro")} placeholder="Ex: 1031/26" className={inputCls} required />
          </div>
        </div>
        ...
        <div className="glass rounded-xl p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <DatePickerField
              ...
              onChange={(d) => setForm(p => ({ ...p, dataExpedicao: d }))}
            />
            <DatePickerField
          ...
          <DatePickerField
            ...
            value={form.terminoCurso}
            onChange={(d) => setForm(p => ({ ...p, terminoCurso: d }))}
          />
        </div>
