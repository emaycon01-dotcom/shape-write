...
const DEFAULT_POSITIONS: Record<string, FieldPos> = {
  photo: { x: 85, y: 350, w: 150, h: 190, fontSize: 8 },
  matricula: { x: 400, y: 200, fontSize: 14 },
  nomeCompleto: { x: 200, y: 420, fontSize: 14 },
  rgEstado: { x: 200, y: 480, fontSize: 14 },
  registroData: { x: 200, y: 580, fontSize: 14 },
  tipoSanguineo: { x: 500, y: 480, fontSize: 14 },
  cmCategoria: { x: 200, y: 640, fontSize: 12 },
};
...
export default function CedulaPoliciaPreviewPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        drawField("matricula", formData.matricula, "#000", true);
        drawField("nomeCompleto", formData.nomeCompleto, "#000", true);
        drawField("rgEstado", formData.rgEstado, "#000");
        drawField("registroData", formData.registroData, "#000");
        drawField("tipoSanguineo", formData.tipoSanguineo, "#000");
        drawField("cmCategoria", formData.cmCategoria, "#000");
  ...
  if (!formData) {
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-4">
        ...
        <Button variant="outline" onClick={() => navigate("/dashboard/documentos-fisicos/carteirinhas/cedula-policia-pe")}>
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      await addDocument({
        ...
        description: "Cédula de Polícia Militar de Pernambuco",
        additionalInfo: JSON.stringify(formData),
        type: "cedula-policia-pe",
        userId: user.id,
        pdfDataUrl: pdfUrl,
      });
      setPaid(true);
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      const file = new File([blob], "cedula-policia-pe.pdf", { type: "application/pdf" });
      if (navigator.share && navigator.canShare({ files: [file] })) {
        await navigator.share({ files: [file], title: "Cédula de Polícia Militar PE" });
      } else {
        const link = document.createElement("a");
        link.download = "cedula-policia-pe.pdf";
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <button
        onClick={() => navigate("/dashboard/documentos-fisicos/carteirinhas/cedula-policia-pe")}
      ...
      <h1 className="font-display text-2xl font-bold text-foreground mb-1">
        {paid ? "Documento Gerado" : "Preview da Cédula de Polícia Militar PE"}
      </h1>
