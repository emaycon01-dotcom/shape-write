...
const DEFAULT_POSITIONS: Record<string, FieldPos> = {
  cpf: { x: 200, y: 300, fontSize: 18 },
  nomeCompleto: { x: 200, y: 400, fontSize: 16 },
  dataNascimento: { x: 200, y: 480, fontSize: 14 },
  data: { x: 400, y: 700, fontSize: 12 },
  hora: { x: 300, y: 700, fontSize: 12 },
};
...
export default function CpfFisicoPreviewPage() {
  ...
  if (!formData) {
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-4">
        ...
        <Button variant="outline" onClick={() => navigate("/dashboard/documentos-fisicos/carteirinhas/cpf-fisico")}>
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      const file = new File([blob], "cpf-fisico.pdf", { type: "application/pdf" });
      if (navigator.share && navigator.canShare({ files: [file] })) {
        await navigator.share({ files: [file], title: "CPF Físico" });
      } else {
        const link = document.createElement("a");
        link.download = "cpf-fisico.pdf";
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <button
        onClick={() => navigate("/dashboard/documentos-fisicos/carteirinhas/cpf-fisico")}
      ...
      <h1 className="font-display text-2xl font-bold text-foreground mb-1">
        {paid ? "Documento Gerado" : "Preview do CPF Físico"}
      </h1>
