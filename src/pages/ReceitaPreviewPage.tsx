...
export default function ReceitaPreviewPage() {
  ...
  useEffect(() => {
    ...
    const id = window.setTimeout(() => {
      prefetchGeneratePdf("generate-receita-pdf", { ...formData, preview: false });
    }, 1200);
  ...
  if (!pdfBase64 || !formData) {
    return (
      <div className="flex flex-col items-center justify-center gap-4 py-20">
        ...
        <Button variant="outline" onClick={() => navigate("/dashboard/documents/receita-medica")}>
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const deduction = await deductCredit(1, "geracao-receita", creditRef("geracao-receita", formData));
      if (!deduction.ok) {
      ...
      await addDocument({
        ...
        date: formData.emissao || "",
        description: `Receita Médica - ${formData.cidade_unidade || ""}`,
  ...
  const handleDownload = () => {
    try {
      ...
      link.href = url;
      link.download = "receita-medica.pdf";
      link.click();
  ...
  const handleShare = async () => {
    try {
      const blob = pdfDataUrlToBlob(pdfBase64);
      if (!blob) throw new Error("Failed to create PDF blob");
      const file = new File([blob], "receita-medica.pdf", { type: "application/pdf" });
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      <h1 className="font-display mb-1 text-2xl font-bold text-foreground">
        {paid ? "Documento Gerado" : "Preview da Receita Médica"}
      </h1>
