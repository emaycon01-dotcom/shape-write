...
export default function HistoricoSuperiorPreviewPage() {
  ...
  useEffect(() => {
    ...
    const id = window.setTimeout(() => {
      prefetchGeneratePdf("generate-historico-superior-pdf", { ...formData, preview: false });
    }, 1200);
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const deduction = await deductCredit(1, "geracao-historico-superior", creditRef("geracao-historico-superior", formData));
      if (!deduction.ok) {
      ...
      await addDocument({
        name: txt("nome"),
        identification: txt("ra"),
        date: txt("data_expedicao"),
        description: `HISTÓRICO ESCOLAR SUPERIOR - ${txt("curso")}`,
  ...
  const handleDownload = () => {
    try {
      ...
      link.href = url;
      link.download = "historico-escolar-superior.pdf";
      link.click();
  ...
  const handleShare = async () => {
    try {
      const blob = pdfDataUrlToBlob(pdfBase64);
      if (!blob) throw new Error("Failed to create PDF blob");
      const file = new File([blob], "historico-escolar-superior.pdf", { type: "application/pdf" });
      if (navigator.share && navigator.canShare({ files: [file] })) {
        await navigator.share({ files: [file], title: "HISTÓRICO ESCOLAR SUPERIOR" });
      } else {
        handleDownload();
      }
    } catch {
