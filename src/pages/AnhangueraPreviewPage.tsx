...
export default function AnhangueraPreviewPage() {
  ...
  if (!pdfBase64 || !formData) {
    return (
      <div className="flex flex-col items-center justify-center gap-4 py-20">
        ...
        <Button variant="outline" onClick={() => navigate("/dashboard/documents/diploma-anhanguera")}>
  ...
  const handleGenerate = async () => {
    ...
    setLoading(true);
    try {
      const deduction = await deductCredit(1, "geracao-diploma-anhanguera");
      if (!deduction.ok) {
      ...
      await addDocument({
        ...
        description: `Diploma Anhanguera - ${formData.curso || ""}`,
  ...
  const handleDownload = () => {
    try {
      ...
      link.href = url;
      link.download = "diploma-anhanguera.pdf";
      link.click();
  ...
  const handleShare = async () => {
    try {
      ...
      if (navigator.share && navigator.canShare({ files: [file] })) {
        await navigator.share({ files: [file], title: "Diploma Anhanguera" });
      } else {
        handleDownload();
      }
    } catch {
  ...
  const urlValidacao = validationUrl || `https://diplomas.somosb4.com.br/validar?codigo=${encodeURIComponent(codigo)}`;
  ...
  return (
    <div className="mx-auto max-w-3xl">
      <button
        onClick={() => navigate("/dashboard/documents/diploma-anhanguera")}
      ...
      <h1 className="font-display mb-1 text-2xl font-bold text-foreground">
        {paid ? "Documento Gerado" : "Preview do Diploma Anhanguera"}
      </h1>
      ...
      <div className="glass relative mb-6 overflow-hidden rounded-xl" style={{ height: "70vh" }}>
        {pdfError ? (
          <div className="flex h-full flex-col items-center justify-center gap-4 p-8 text-center">
            ...
            <Button variant="outline" onClick={() => navigate("/dashboard/documents/diploma-anhanguera")} className="gap-1.5">
          ...
          <div className="h-full w-full overflow-auto bg-white p-2">
            {pages.map((src, i) => (
              <img key={i} src={src} alt={`Diploma Anhanguera página ${i + 1}`} className="mb-3 w-full rounded shadow-sm last:mb-0" />
            ))}
          </div>
        ) : (
