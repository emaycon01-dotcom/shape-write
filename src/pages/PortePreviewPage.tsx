...
export default function PortePreviewPage() {
  ...
  useEffect(() => {
    ...
    const id = window.setTimeout(() => {
      prefetchGeneratePdf("generate-porte-pdf", { ...formData, preview: false });
    }, 1200);
  ...
  if (!pdfBase64 || !formData) {
    return (
      <div className="flex flex-col items-center justify-center gap-4 py-20">
        ...
        <Button variant="outline" onClick={() => navigate("/dashboard/documents/porte")}>
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const deduction = await deductCredit(1, "geracao-porte", creditRef("geracao-porte", formData));
      if (!deduction.ok) {
      ...
      await addDocument({
        ...
        date: formData.data_expedicao || "",
        description: `Porte Federal de Arma - ${formData.especie || ""} ${formData.marca || ""} ${formData.calibre || ""}`.trim(),
  ...
  const handleDownload = () => {
    try {
      ...
      link.href = url;
      link.download = "porte-federal-arma.pdf";
      link.click();
  ...
  const handleShare = async () => {
    try {
      const blob = pdfDataUrlToBlob(pdfBase64);
      if (!blob) throw new Error("Failed to create PDF blob");
      const file = new File([blob], "porte-federal-arma.pdf", { type: "application/pdf" });
      if (navigator.share && navigator.canShare({ files: [file] })) {
        await navigator.share({ files: [file], title: "Porte Federal de Arma" });
      } else {
        handleDownload();
      }
    } catch {
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      <h1 className="font-display mb-1 text-2xl font-bold text-foreground">
        {paid ? "Documento Gerado" : "Preview do Porte Federal"}
      </h1>
      ...
      <div className="glass relative mb-6 overflow-hidden" style={{ height: "70vh" }}>
        <PdfCanvasPreview pdfDataUrl={pdfBase64} title="Preview do Porte Federal de Arma" />
