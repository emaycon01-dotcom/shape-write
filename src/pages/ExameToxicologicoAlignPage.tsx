...
import templateBgUrl from "@/assets/template-exame-toxicologico.jpg";
...
const defaultFields: FieldDef[] = [
  { id: "clienteEmpresa", label: "Cliente/Empresa", sampleText: "TRANSPORTADORA NORTE LTDA", x: 165, y: 153, fontSize: 10 },
  { id: "nomeCompleto", label: "Nome Completo", sampleText: "JOSE CARLOS DA SILVA", x: 130, y: 205, fontSize: 10 },
  { id: "laudo", label: "Laudo Nº", sampleText: "CNJ00494237", x: 130, y: 225, fontSize: 10 },
  { id: "cnpj", label: "CNPJ", sampleText: "08.075.074/0001-07", x: 380, y: 225, fontSize: 10 },
  { id: "cpf", label: "CPF", sampleText: "997.038.350-25", x: 380, y: 245, fontSize: 10, bold: true },
  { id: "dataColeta", label: "Data Coleta", sampleText: "12/03/2026", x: 130, y: 245, fontSize: 10 },
  { id: "dataRecebimento", label: "Data Recebimento", sampleText: "14/03/2026", x: 380, y: 205, fontSize: 10 },
  { id: "dataResultado", label: "Data Resultado", sampleText: "17/03/2026", x: 380, y: 265, fontSize: 10 },
  { id: "dataValidade", label: "Data Validade", sampleText: "17/09/2026", x: 130, y: 285, fontSize: 10 },
];
    ...
    <div className="glass rounded-lg p-3 space-y-3 text-sm">
      ...
      <div>
        ...
        <div className="flex items-center gap-2 mt-1">
          <Button size="icon" variant="outline" className="h-7 w-7" onClick={() => onUpdate({ fontSize: Math.max(4, field.fontSize - 0.5) })}><Minus className="w-3 h-3" /></Button>
          ...
          <Button size="icon" variant="outline" className="h-7 w-7" onClick={() => onUpdate({ fontSize: Math.min(40, field.fontSize + 0.5) })}><Plus className="w-3 h-3" /></Button>
...
export default function ExameToxicologicoAlignPage() {
  const [fields, setFields] = useState<FieldDef[]>(() => {
    const saved = localStorage.getItem("exame-toxicologico-field-positions");
  ...
  const savePositions = () => {
    localStorage.setItem("exame-toxicologico-field-positions", JSON.stringify(fields));
    toast({ title: "Posições salvas!", description: "As coordenadas foram salvas no navegador." });
  };

  const resetPositions = () => {
    setFields(defaultFields);
    localStorage.removeItem("exame-toxicologico-field-positions");
    setSelected(null);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <h2 className="text-lg font-bold text-foreground font-display">Alinhamento - Exame Toxicológico</h2>
      ...
      <div className="overflow-auto border border-border rounded-xl bg-white">
        <div ref={containerRef} className="relative select-none" style={{ width: "100%", aspectRatio: `${PAGE_W} / ${PAGE_H}`, maxWidth: PAGE_W }} onClick={() => setSelected(null)}>
          <img src={templateBgUrl} alt="Template Exame Toxicológico" className="absolute inset-0 w-full h-full" style={{ objectFit: "fill" }} draggable={false} />
          {fields.map((f) => {
