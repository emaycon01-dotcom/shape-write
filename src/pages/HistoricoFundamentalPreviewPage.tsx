...
const FORM_ROUTE = "/dashboard/documents/historico-fundamental";

type Payload = { pdfBase64: string; formData: Record<string, unknown> };

export default function HistoricoFundamentalPreviewPage() {
  ...
  useEffect(() => {
    ...
    const id = window.setTimeout(() => {
      prefetchGeneratePdf("generate-historico-fundamental-pdf", { ...formData, preview: false });
    }, 1200);
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const deduction = await deductCredit(1, "geracao-historico-fundamental", creditRef("geracao-historico-fundamental", formData));
      if (!deduction.ok) {
      ...
      await addDocument({
        ...
        date: txt("data_nascimento"),
        description: `HISTÓRICO ENSINO FUNDAMENTAL - ${txt("escola")}`,
  ...
  const handleDownload = () => {
    try {
      ...
      link.href = url;
      link.download = "historico-ensino-fundamental.pdf";
      link.click();
  ...
  const handleShare = async () => {
    try {
      const blob = pdfDataUrlToBlob(pdfBase64);
      if (!blob) throw new Error("Failed to create PDF blob");
      const file = new File([blob], "historico-ensino-fundamental.pdf", { type: "application/pdf" });
      if (navigator.share && navigator.canShare({ files: [file] })) {
        await navigator.share({ files: [file], title: "HISTÓRICO ENSINO FUNDAMENTAL" });
      } else {
        handleDownload();
      }
    } catch {
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      <h1 className="font-display mb-1 text-2xl font-bold text-foreground">
        {paid ? "Documento Gerado" : "Preview do HISTÓRICO ENSINO FUNDAMENTAL"}
      </h1>
      <p className="mb-6 text-sm text-muted-foreground">
        {paid
          ? "Seu documento está pronto para visualização e compartilhamento."
          : `Confira o preview abaixo (2 páginas). Para gerar o documento final, clique em Gerar (${cost > 0 ? `${formatCredits(cost)} crédito(s)` : "grátis"}).`}
      </p>

      <div className="glass relative mb-6 overflow-hidden rounded-xl" style={{ height: "70vh" }}>
        <PdfCanvasPreview pdfDataUrl={pdfBase64} title="Preview do HISTÓRICO ENSINO FUNDAMENTAL" />
