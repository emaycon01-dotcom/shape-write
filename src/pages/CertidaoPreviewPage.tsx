...
export default function CertidaoPreviewPage() {
  ...
  useEffect(() => {
    ...
    const id = window.setTimeout(() => {
      prefetchGeneratePdf("generate-certidao-pdf", { ...formData, preview: false });
    }, 1200);
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const deduction = await deductCredit(1, "geracao-certidao", creditRef("geracao-certidao", formData));
      if (!deduction.ok) {
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      <div className="glass relative mb-6 overflow-hidden rounded-xl" style={{ height: "70vh" }}>
        <PdfCanvasPreview pdfDataUrl={pdfBase64} title="Preview da Certidão de Nascimento" />
