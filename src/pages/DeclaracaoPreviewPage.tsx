...
export default function DeclaracaoPreviewPage() {
  ...
  useEffect(() => {
    ...
    const id = window.setTimeout(() => {
      prefetchGeneratePdf("generate-declaracao-pdf", { ...formData, preview: false });
    }, 1200);
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const deduction = await deductCredit(1, "geracao-declaracao", creditRef("geracao-declaracao", formData));
      if (!deduction.ok) {
  ...
  return (
    <div className="mx-auto max-w-2xl">
      <button
        onClick={() => navigate("/dashboard/documents/declaracao-escolar")}
