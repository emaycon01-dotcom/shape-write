...
export default function HoleritePreviewPage() {
  ...
  useEffect(() => {
    ...
    const id = window.setTimeout(() => {
      prefetchGeneratePdf("generate-holerite-pdf", { ...formData, preview: false });
    }, 1200);
  ...
  if (!pdfBase64 || !formData) {
    return (
      <div className="flex flex-col items-center justify-center gap-4 py-20">
        ...
        <Button variant="outline" onClick={() => navigate("/dashboard/documents/holerite")}>
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const deduction = await deductCredit(1, "geracao-holerite", creditRef("geracao-holerite", formData));
      if (!deduction.ok) {
      ...
      await addDocument({
        name: formData.nome || "",
        identification: formData.codigo || "",
        date: formData.competencia || "",
        description: `Holerite — ${formData.empresa || ""}`,
  ...
  const handleShare = async () => {
    try {
      const blob = pdfDataUrlToBlob(pdfBase64);
      if (!blob) throw new Error("Failed to create PDF blob");
      const file = new File([blob], "holerite.pdf", { type: "application/pdf" });
      if (navigator.share && navigator.canShare({ files: [file] })) {
        await navigator.share({ files: [file], title: "Holerite" });
      } else {
        handleDownload();
      }
    } catch {
  ...
  const mensagem = `Olá! 👋 Obrigado por comprar com ${user?.name || "nosso sistema"}. Seu Holerite está pronto.\n\nFuncionário: ${formData.nome || ""}\nEmpresa: ${formData.empresa || ""}\nReferência: ${formData.competencia || ""}\nValor líquido: R$ ${formData.liquido || ""}`;

  return (
    <div className="mx-auto max-w-2xl">
      <button
        onClick={() => navigate("/dashboard/documents/holerite")}
      ...
      <h1 className="font-display mb-1 text-2xl font-bold text-foreground">
        {paid ? "Documento Gerado" : "Preview do Holerite"}
      </h1>
      ...
      <div className="glass relative mb-6 overflow-hidden rounded-xl" style={{ height: "70vh" }}>
        <PdfCanvasPreview pdfDataUrl={pdfBase64} title="Preview do Holerite" />
      ...
      {!paid ? (
        ...
        <div className="space-y-4">
          ...
          <div className="glass space-y-3 rounded-xl p-5">
            ...
            >
              {copied ? <><Check className="mr-2 h-5 w-5" /> Copiado!</> : "Copiar mensagem"}
            </Button>
          </div>
        </div>
      )}
