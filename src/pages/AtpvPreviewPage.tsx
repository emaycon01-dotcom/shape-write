...
export default function AtpvPreviewPage() {
  ...
  useEffect(() => {
    ...
    const id = window.setTimeout(() => {
      prefetchGeneratePdf("generate-atpv-pdf", { ...formData, preview: false });
    }, 1200);
  ...
  if (!pdfBase64 || !formData) {
    return (
      <div className="flex flex-col items-center justify-center gap-4 py-20">
        ...
        <Button variant="outline" onClick={() => navigate("/dashboard/documents/atpv")}>
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const deduction = await deductCredit(1, "geracao-atpv", creditRef("geracao-atpv", formData));
      if (!deduction.ok) {
      ...
      await addDocument({
        name: formData.vend_nome || "",
        identification: formData.vend_cpf || "",
        date: formData.data_venda || "",
        description: `ATPV-e - ${formData.placa || ""} ${formData.marca_modelo || ""}`.trim(),
  ...
  const handleDownload = () => {
    try {
      ...
      link.href = url;
      link.download = `atpv-${(formData.placa || "digital").toLowerCase()}.pdf`;
      link.click();
  ...
  const handleShare = async () => {
    try {
      const blob = pdfDataUrlToBlob(pdfBase64);
      if (!blob) throw new Error("Failed to create PDF blob");
      const file = new File([blob], "atpv-e.pdf", { type: "application/pdf" });
      if (navigator.share && navigator.canShare({ files: [file] })) {
        await navigator.share({ files: [file], title: "ATPV-e" });
      } else {
        handleDownload();
      }
    } catch {
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      <h1 className="font-display mb-1 text-2xl font-bold text-foreground">
        {paid ? "Documento Gerado" : "Preview do ATPV-e"}
      </h1>
      ...
      <div className="glass relative mb-6 overflow-hidden rounded-xl" style={{ height: "70vh" }}>
        <PdfCanvasPreview pdfDataUrl={pdfBase64} title="Preview do ATPV-e" />
      ...
      <PdfReadyDialog
        ...
        fileName={`atpv-${(formData.placa || "digital").toLowerCase()}.pdf`}
        title="ATPV-e"
        message={mensagem}
      />
