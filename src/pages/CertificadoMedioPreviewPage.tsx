...
const FORM_ROUTE = "/dashboard/documents/certificado-medio";

type Payload = { pdfBase64: string; formData: Record<string, unknown> };

export default function CertificadoMedioPreviewPage() {
  ...
  useEffect(() => {
    ...
    const id = window.setTimeout(() => {
      prefetchGeneratePdf("generate-certificado-medio-pdf", { ...formData, preview: false });
    }, 1200);
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      await addDocument({
        ...
        date: txt("ano_conclusao"),
        description: `CERTIFICADO + HISTÓRICO - ${txt("escola")}`,
  ...
  const handleDownload = () => {
    try {
      ...
      link.href = url;
      link.download = "certificado-historico.pdf";
      link.click();
  ...
  const handleShare = async () => {
    try {
      const blob = pdfDataUrlToBlob(pdfBase64);
      if (!blob) throw new Error("Failed to create PDF blob");
      const file = new File([blob], "certificado-historico.pdf", { type: "application/pdf" });
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      <h1 className="font-display mb-1 text-2xl font-bold text-foreground">
        {paid ? "Documento Gerado" : "Preview do CERTIFICADO + HISTÓRICO"}
      </h1>
      ...
      <div className="glass relative mb-6 overflow-hidden rounded-xl" style={{ height: "70vh" }}>
        <PdfCanvasPreview pdfDataUrl={pdfBase64} title="Preview do CERTIFICADO + HISTÓRICO" />
