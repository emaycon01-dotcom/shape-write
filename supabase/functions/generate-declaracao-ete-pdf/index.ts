...
// Defaults MUST match src/pages/TemplateAlignPage.tsx defaultDeclaracaoEteFields
export const DECLARACAO_ETE_DEFAULT_POSITIONS: Record<string, Pos> = {
  corpo: { x: 86, y: 355, fontSize: 16, w: 637 },
  data_local: { x: 393, y: 545, fontSize: 16, w: 330 },
};

function resolvePositions(overrides: unknown): Record<string, Pos> {
  const result: Record<string, Pos> = { ...DECLARACAO_ETE_DEFAULT_POSITIONS };
...
/** Corpo da declaração de matrícula da Escola Técnica Estadual. */
export function buildCorpoEteHtml(d: Record<string, string>): string {
  const b = (v: string) => `<b>${escapeHtml((v || "").trim())}</b>`;
  const t = (v: string) => escapeHtml((v || "").trim());

  return (
    `<span style="display:inline-block;width:56px;"></span>` +
    `Declaramos para os devidos fins de comprovação que ${b(d.nome_aluno)}, ` +
    `portador (a) da cédula de identidade nº ${t(d.rg)} ${t(d.orgao_emissor)} e C.P.F nº ${t(d.cpf)}, ` +
    `filho (a) de ${t(d.pai)} e de ${t(d.mae)}, está regularmente matriculado (a) no ` +
    `${t(d.serie)} do ${t(d.nivel_ensino)}, no curso Técnico em ${b(d.curso)}, ` +
    `na modalidade ${t(d.modalidade)}, onde cursa de ${t(d.dias_semana)}, ` +
    `no horário das ${t(d.horario)} nesta unidade de ensino.`
  );
}

/* --------------------------------------------------------------- layout */

export function buildDeclaracaoEteHtml(d: Record<string, string>, fieldPositions?: unknown) {
...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .overlay { position: absolute; z-index: 10; color: #000; line-height: 1.2; white-space: normal; overflow: visible; }
...
<div class="page">
  ...
  <div class="overlay corpo" style="top:${p.corpo.y}px;left:${p.corpo.x}px;width:${p.corpo.w ?? 637}px;font-size:${p.corpo.fontSize}px;">${buildCorpoEteHtml(d)}</div>
...
serve(async (req) => {
  ...
  try {
    ...
    const keys = [
      ...
      "serie", "nivel_ensino", "curso", "modalidade",
      "dias_semana", "horario", "cidade", "data_emissao",
    ];
    ...
    const html = buildDeclaracaoEteHtml(data, body.field_positions);
    ...
    console.error("Error generating Declaração ETE:", error);
